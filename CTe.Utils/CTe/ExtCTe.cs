...
public static class ExtCTe {
  ...
  public static CteEletronica CarregarDeArquivoXml(this CteEletronica cte,
                                                   string arquivoXml) {
    var s = FuncoesXml.ObterNodeDeArquivoXml(typeof(CteEletronica).Name,
                                             arquivoXml);
  ...
  public static CteEletronica CarregarDeXmlString(this CteEletronica cte,
                                                  string xmlString) {
    var s =
        FuncoesXml.ObterNodeDeStringXml(typeof(CteEletronica).Name, xmlString);
    return FuncoesXml.XmlStringParaClasse<CteEletronica>(s);
  }

  /// <summary>
  ///     Gera id, cdv, assina e faz alguns ajustes nos dados da classe CTe
  ///     antes de utilizá-la
  /// </summary>
  /// <param name="cte"></param>
  ...
  public static void
  ValidaSchema(this CteEletronica cte,
    ...
    default:
      throw new InvalidOperationException(
          "Nos achamos um erro na hora de validar o schema, " +
          "a versão está inválida, somente é permitido " +
          "versão 2.00 é 3.00");
    }

    if (cte.infCte.ide.tpCTe != tpCTe.Anulacao &&
        cte.infCte.ide.tpCTe !=
            tpCTe.Complemento) // Ct-e do Tipo Anulação/Complemento não tem
                               // Informações do Modal
    {
      var xmlModal =
          FuncoesXml.ClasseParaXmlString(cte.infCte.infCTeNorm.infModal);

      switch (cte.infCte.infCTeNorm.infModal.versaoModal) {
      case versaoModal.veM200:
        if (cte.infCte.infCTeNorm.infModal.ContainerModal.GetType() ==
            typeof(aereo)) {
      ...
      case versaoModal.veM300:
        ...
        if (cte.infCte.infCTeNorm.infModal.ContainerModal.GetType() ==
            typeof(rodoOS)) {
          Validador.Valida(xmlModal, "cteModalRodoviarioOS_v3.00.xsd",
                           servicoInstancia);
        }
        break;
      case versaoModal.veM400:
        if (cte.infCte.infCTeNorm.infModal.ContainerModal.GetType() ==
            typeof(aereo)) {
          Validador.Valida(xmlModal, "cteModalAereo_v4.00.xsd",
                           servicoInstancia);
        }

        if (cte.infCte.infCTeNorm.infModal.ContainerModal.GetType() ==
            typeof(aquav)) {
          Validador.Valida(xmlModal, "cteModalAquaviario_v4.00.xsd",
                           servicoInstancia);
        }

        if (cte.infCte.infCTeNorm.infModal.ContainerModal.GetType() ==
            typeof(duto)) {
          Validador.Valida(xmlModal, "cteModalDutoviario_v4.00.xsd",
                           servicoInstancia);
        }

        if (cte.infCte.infCTeNorm.infModal.ContainerModal.GetType() ==
            typeof(ferrov)) {
          Validador.Valida(xmlModal, "cteModalFerroviario_v4.00.xsd",
                           servicoInstancia);
        }

        if (cte.infCte.infCTeNorm.infModal.ContainerModal.GetType() ==
            typeof(rodo)) {
          Validador.Valida(xmlModal, "cteModalRodoviario_v4.00.xsd",
                           servicoInstancia);
        }

        if (cte.infCte.infCTeNorm.infModal.ContainerModal.GetType() ==
            typeof(multimodal)) {
          Validador.Valida(xmlModal, "cteMultimodal_v4.00.xsd",
                           servicoInstancia);
        }

        if (cte.infCte.infCTeNorm.infModal.ContainerModal.GetType() ==
            typeof(rodoOS)) {
          Validador.Valida(xmlModal, "cteModalRodoviarioOS_v4.00.xsd",
                           servicoInstancia);
        }
        break;
      default:
  ...
  public static void Assina(this CteEletronica cte,
    ...
    var dadosChave = ChaveFiscal.ObterChave(
        estado, dataEHoraEmissao, cnpj, modeloDocumentoFiscal, serie,
        numeroDocumento, tipoEmissao, codigoNumerico);
    ...
    var assinatura = AssinaturaDigital.Assina(cte, cte.infCte.Id,
  ...
  public static infCTeSupl QrCode(this CteEletronica cte,
                                  X509Certificate2 certificadoDigital,
                                  Encoding encoding, bool isAdicionaQrCode,
                                  string url) {
    if (isAdicionaQrCode == false)
      return null;
    ...
    if (cte.infCte.ide.tpEmis != tpEmis.teNormal &&
      ...
      var assinatura = Convert.ToBase64String(
          CreateSignaturePkcs1(certificadoDigital, encoding.GetBytes(chave)));
      qrCode.Append("&sign=");
      qrCode.Append(assinatura);
    }

    return new infCTeSupl { qrCodCTe = qrCode.ToString() };
  }

  private static byte[] CreateSignaturePkcs1(
      X509Certificate2 certificadoDigital, byte[] Value) {
  ...
  public static void
  SalvarXmlEmDisco(this CteEletronica cte,
                   ConfiguracaoServico configuracaoServico = null) {
  ...
  CriaRequestWs(this CteEletronica cte,
                ConfiguracaoServico configuracaoServico = null) {
    ...
    if (instanciaServico.cUF == Estado.PR || instanciaServico.cUF == Estado.MT)
      // Caso o lote seja enviado para o PR, colocar o namespace nos elementos
      // <CTe> do lote, pois o serviço do PR o exige, conforme
      // https://github.com/adeniltonbs/Zeus.Net.NFe.NFCe/issues/456
      xml = xml.Replace("<CTe>",
  ...
  public static void
  SalvarXmlEmDisco(this retCTe retEnviCte, string chave,
                   ConfiguracaoServico configuracaoServico = null) {
    var instanciaServico = configuracaoServico ?? ConfiguracaoServico.Instancia;
