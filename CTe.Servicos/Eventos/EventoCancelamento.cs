...
public class EventoCancelamento {
  ...
  public EventoCancelamento(CteEletronico cte, int sequenciaEvento,
                            string numeroProtocolo, string justificativa) {
    _cte = cte;
  ...
  public EventoCancelamento(CteEletronicoOS cte, int sequenciaEvento,
                            string numeroProtocolo, string justificativa) {
    _cteOs = cte;
    _sequenciaEvento = sequenciaEvento;
    _numeroProtocolo = numeroProtocolo;
  ...
  public retEventoCTe Cancelar(ConfiguracaoServico configuracaoServico = null) {
    ...
    EventoEnviado = FactoryEvento.CriaEvento(
        CTeTipoEvento.Cancelamento, _sequenciaEvento, _cte.Chave(),
        _cte.infCte.emit.CNPJ, evento, configServico);
    RetornoSefaz = new ServicoController().Executar(
        _cte, _sequenciaEvento, evento, CTeTipoEvento.Cancelamento,
        configServico);
  ...
  public retEventoCTe
  CancelarOs(ConfiguracaoServico configuracaoServico = null) {
    var configServico = configuracaoServico ?? ConfiguracaoServico.Instancia;
    var evento = ClassesFactory.CriaEvCancCTe(_justificativa, _numeroProtocolo);

    EventoEnviado = FactoryEvento.CriaEvento(
        CTeTipoEvento.Cancelamento, _sequenciaEvento, _cteOs.Chave(),
        _cteOs.InfCte.emit.CNPJ, evento, configServico);
    RetornoSefaz = new ServicoController().Executar(
        _cteOs, _sequenciaEvento, evento, CTeTipoEvento.Cancelamento,
        configServico);
  ...
  CancelarAsync(ConfiguracaoServico configuracaoServico = null) {
    var configServico = configuracaoServico ?? ConfiguracaoServico.Instancia;
    var evento = ClassesFactory.CriaEvCancCTe(_justificativa, _numeroProtocolo);
    ...
    EventoEnviado = FactoryEvento.CriaEvento(
        CTeTipoEvento.Cancelamento, _sequenciaEvento, _cte.Chave(),
        _cte.infCte.emit.CNPJ, evento, configServico);
    RetornoSefaz = await new ServicoController().ExecutarAsync(
        _cte, _sequenciaEvento, evento, CTeTipoEvento.Cancelamento,
        configServico);
