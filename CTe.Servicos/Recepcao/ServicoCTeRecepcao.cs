...
public class ServicoCTeRecepcao {
  ...
  public retEnviCte
  CTeRecepcao(int lote, List<CTeEletronico> cteEletronicosList,
    ...
    var retornoXml =
        webService.cteRecepcaoLote(enviCte.CriaRequestWs(configServico));
  ...
  public async Task<retEnviCte>
  CTeRecepcaoAsync(int lote, List<CTeEletronico> cteEletronicosList,
                   ConfiguracaoServico configuracaoServico = null) {
    var configServico = configuracaoServico ?? ConfiguracaoServico.Instancia;
    ...
    var retornoXml = await webService.cteRecepcaoLoteAsync(
        enviCte.CriaRequestWs(configServico));
  ...
  public retCTe
  CTeRecepcaoSincronoV4(CTeEletronico cte,
                        ConfiguracaoServico configuracaoServico = null) {
    var instanciaConfiguracao =
        configuracaoServico ?? ConfiguracaoServico.Instancia;

    if (instanciaConfiguracao.tpAmb == TipoAmbiente.Homologacao &&
        instanciaConfiguracao.VersaoLayout == versao.ve300) {
      const string razaoSocial =
    ...
    if (instanciaConfiguracao.tpAmb == TipoAmbiente.Homologacao &&
        instanciaConfiguracao.VersaoLayout == versao.ve400) {
      const string razaoSocial =
    ...
    cte.infCTeSupl =
        cte.QrCode(instanciaConfiguracao.X509Certificate2, Encoding.UTF8,
                   instanciaConfiguracao.IsAdicionaQrCode,
                   UrlHelper.ObterUrlQrCode(instanciaConfiguracao));
    cte.SalvarXmlEmDisco(
        instanciaConfiguracao); // salva em disco antes de validas os schemas,
                                // facilitando encontrar possíveis erros
    cte.ValidaSchema(instanciaConfiguracao);
    cte.SalvarXmlEmDisco(instanciaConfiguracao);

    var webService =
        WsdlFactory.CriaWsdlCteRecepcaoSincronoV4(instanciaConfiguracao);

    // OnAntesDeEnviar(enviCte);

    var retornoXml =
        webService.CTeRecepcaoSincV4(cte.CriaRequestWs(instanciaConfiguracao));
  ...
  private static enviCTe
  PreparaEnvioCTe(int lote, List<CTeEletronico> cteEletronicosList,
                  ConfiguracaoServico configuracaoServico = null) {
    var instanciaConfiguracao =
        configuracaoServico ?? ConfiguracaoServico.Instancia;
